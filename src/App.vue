...
//todo rellenar con 00 para que no quede vacio
  const recorrer = () => {
    ...
    switch (opcode) {
      ...
      break;
      case "0000": //br
        if ((n==="1" && signo.value===-1) || (z==="1" && signo.value===0) || (p==="1" && signo.value===1))
      ...
      case "0100": //jsr jsrr
        let temp = pc.value
        pc.value = parseInt(insBin.substring(4, 5),2) === 0 ? registros.value[baseR] : (pc.value + PCoffset11)
...
function hexadecimalADecimalConSigno(hex) {
  ...
  {
    memoriaRam.value = new Array(32).fill(0)
  }
  ...
  <div class="row">
    ...
    <div class="col-4">
      ...
      <div class="row justify-content-between">
        <button @click="limpiarMemoria" type="button" class="col btn btn-danger m-2">Limpiar memoria
        </button>
      ...
      <table class="table table-hover">
        ...
        <tbody>
          <tr @click="pc=index" v-for="(item, index) in memoriaRam" :class="[pc==index ? 'table-primary' :  'table-ligth']">
            <th scope="row">x{{decimalASignoHexadecimal(index)}}</th>
            <td>{{item}}</td>
            <!--<td>proximamente</td>-->
          </tr>
        </tbody>
    ...
    <div class="col-3">
      <div class="d-grid gap-2">
        ...
        <button v-on:click="limpiarRegistros()" type="button" class="btn btn-danger">Reiniciar registros</button>
...
<footer>
  Parra Cristian V.R. -- Laboratorio 5 -- Arquitectura de Computadoras
</footer>
